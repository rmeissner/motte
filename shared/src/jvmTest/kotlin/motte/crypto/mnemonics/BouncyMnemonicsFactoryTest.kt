package motte.crypto.mnemonics
...
import dev.rimeissner.motte.mnemonics.ENTROPY_MULTIPLE
import dev.rimeissner.motte.mnemonics.InvalidEntropy
import dev.rimeissner.motte.mnemonics.MAX_ENTROPY_BITS
import dev.rimeissner.motte.mnemonics.MIN_ENTROPY_BITS
import dev.rimeissner.motte.mnemonics.MnemonicNotInWordlist
...
class BouncyMnemonicsFactoryTest {
    private lateinit var factory: BouncyMnemonicsFactory

    @Before
    fun setup() {
        factory = BouncyMnemonicsFactory(ENGLISH_WORD_LIST)
    }

    @Test
    fun mnemonicToSeed() {
        MNEMONIC_TO_SEED.forEach { (mnemonic, seed) ->
            assertEquals(seed, factory.toSeed(mnemonic).toHexString())
        }
    }

    @Test
    fun testGenerateMnemonic() {
        val mnemonic12 = factory.generate()
        assertEquals(12, mnemonic12.words().size)
        assertEquals(mnemonic12, factory.validate(mnemonic12))

        val mnemonic15 = factory.generate(MIN_ENTROPY_BITS + 1 * ENTROPY_MULTIPLE)
        assertEquals(15, mnemonic15.words().size)
        assertEquals(mnemonic15, factory.validate(mnemonic15))

        val mnemonic18 = factory.generate(MIN_ENTROPY_BITS + 2 * ENTROPY_MULTIPLE)
        assertEquals(18, mnemonic18.words().size)
        assertEquals(mnemonic18, factory.validate(mnemonic18))

        val mnemonic21 = factory.generate(MIN_ENTROPY_BITS + 3 * ENTROPY_MULTIPLE)
        assertEquals(21, mnemonic21.words().size)
        assertEquals(mnemonic21, factory.validate(mnemonic21))

        val mnemonic24 = factory.generate(MAX_ENTROPY_BITS)
        assertEquals(24, mnemonic24.words().size)
        assertEquals(mnemonic24, factory.validate(mnemonic24))
    }

    @Test
    fun generateMnemonicInvalidEntropy() {
        assertThrows(IllegalArgumentException::class.java) { factory.generate(0) }
        assertThrows(IllegalArgumentException::class.java) { factory.generate(127) }
        assertThrows(IllegalArgumentException::class.java) { factory.generate(129) }
        assertThrows(IllegalArgumentException::class.java) { factory.generate(257) }
    }
    ...
    fun testValidateMnemonic() {
        (MNEMONICS_ENGLISH_12 + MNEMONICS_ENGLISH_15 + MNEMONICS_ENGLISH_18 + MNEMONICS_ENGLISH_21 +
            ...
            assertEquals(it, factory.validate(it))
        }
    }
    ...
    fun testInvalidMnemonics() {
        (INVALID_MNEMONICS_ENGLISH_12 + INVALID_MNEMONICS_ENGLISH_15 + INVALID_MNEMONICS_ENGLISH_18 +
            ...
            assertThrows(IllegalArgumentException::class.java) { factory.validate(it) }
        }
    }

    @Test
    fun validateEmptyMnemonic() {
        assertThrows(IllegalArgumentException::class.java) { factory.validate("") }
        assertThrows(IllegalArgumentException::class.java) { factory.validate("    ") }
        assertThrows(IllegalArgumentException::class.java) { factory.validate("\n") }
        assertThrows(IllegalArgumentException::class.java) { factory.validate("\t") }
    }
    ...
    fun validateInvalidEntropy() {
        assertThrows(InvalidEntropy::class.java) { factory.validate("page") }
        assertThrows(InvalidEntropy::class.java) { factory.validate("page tuition excess kidney") }
        assertThrows(InvalidEntropy::class.java) { factory.validate("page tuition excess kidney forward collect fashion finger raven honey tiny") }
    }

    @Test
    fun validateMnemonicWithInvalidWord() {
        assertThrows(MnemonicNotInWordlist::class.java) { factory.validate("page tuition excess kidney forward collect fashion finger raven honey tiny gnosis") }
    }
